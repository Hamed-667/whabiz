...
const WHATSAPP_NUMBER = '22654576629'; // Remplace par ton vrai numéro

// Variable pour savoir si on est en mode "modification"
let editMode = false;
...
function displayProducts(products) {
  ...
  products.forEach(product => {
    ...
    const imageSafe = product.image.replace(/'/g, "\\'");
    ...
    productCard.innerHTML = `
      ...
      <div class="product-info">
        ...
        <button class="edit-btn" onclick="editProduct(${product.id}, '${nameSafe}', ${product.price}, '${imageSafe}')">
          ✏️ Modifier
        </button>
...
function editProduct(id, name, price, image) {
  ...
  document.querySelector('.add-product h2').textContent = '✏️ Modifier le produit';
  document.querySelector('#productForm button[type="submit"]').textContent = 'Enregistrer les modifications';
...
function cancelEdit() {
  ...
  document.getElementById('productForm').reset();
  document.querySelector('.add-product h2').textContent = '➕ Ajouter un produit';
  document.querySelector('#productForm button[type="submit"]').textContent = 'Ajouter';
