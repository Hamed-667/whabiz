...
async function loadData() {
  ...
  try {
    ...
    var exportLink = document.getElementById('exportProductsCsv');
    if (exportLink && vendeurId) {
      exportLink.href = '/api/exports/products.csv?vendeurId=' + encodeURIComponent(vendeurId);
    }
...
function mapHeaderKey(value) {
  ...
  if (!key) return '';
  if (key === 'productid' || key === 'id') return 'productId';
  if (key === 'nom' || key === 'name' || key === 'productname') return 'nom';
  if (key === 'categorie' || key === 'category') return 'categorie';
  if (key === 'prix' || key === 'price') return 'prix';
  if (key === 'stock') return 'stock';
  if (key === 'description' || key === 'desc') return 'description';
  if (key === 'image' || key === 'imageurl' || key === 'mainimage') return 'image';
  if (key === 'images' || key === 'imageurls') return 'images';
  if (key === 'prixavant' || key === 'pricebefore' || key === 'oldprice') return 'prixAvant';
  if (key === 'lowstockthreshold' || key === 'lowstock' || key === 'threshold') return 'lowStockThreshold';
  if (key === 'variants' || key === 'options') return 'variants';
  return '';
}
...
function parseVariantsString(value) {
  ...
  return raw.split('|').map(function(entry, index) {
    ...
    if (!name) return null;
    var stock = normalizeStockValue(parts[1]);
    var id = String(parts[2] || '').trim();
    if (!id) id = 'v-' + (index + 1);
    return { id: id, name: name, stock: stock };
  }).filter(Boolean);
}

function parseImagesString(value) {
  var raw = String(value || '').trim();
  if (!raw) return [];
...
async function handleCsvImport() {
  ...
  if (!input || !input.files || !input.files.length) {
    setImportStatus('Selectionnez un fichier CSV.', 'error');
    return;
  }
  ...
  try {
    ...
    setImportStatus('Impossible de lire le fichier CSV.', 'error');
    return;
  }
  var rows = parseCsv(text);
  if (rows.length < 2) {
    setImportStatus('CSV vide ou invalide.', 'error');
    return;
  }
  var headers = rows.shift().map(mapHeaderKey);
  if (!headers.some(Boolean)) {
    setImportStatus('Colonnes CSV non reconnues.', 'error');
    return;
  }
  var payloads = [];
  var errors = [];
  ...
  rows.forEach(function(row, rowIndex) {
    var record = {};
    headers.forEach(function(key, colIndex) {
      if (!key) return;
      var value = row[colIndex] !== undefined ? String(row[colIndex]).trim() : '';
      if (value !== '') record[key] = value;
    });
    if (!Object.keys(record).length) return;

    var productId = record.productId ? String(record.productId).trim() : '';
    var nom = record.nom ? String(record.nom).trim() : '';
    var prix = record.prix !== undefined ? parseFloat(String(record.prix).replace(',', '.')) : null;

    if (!productId && (!nom || !Number.isFinite(prix))) {
      errors.push('Ligne ' + (rowIndex + 2) + ': nom/prix requis pour creer.');
      return;
    }

    var payload = {};
    if (nom) payload.nom = nom;
    if (record.categorie) payload.categorie = record.categorie;
    if (record.description) payload.description = record.description;
    if (Number.isFinite(prix)) payload.prix = prix;

    if (record.prixAvant !== undefined) {
      var prixAvant = parseFloat(String(record.prixAvant).replace(',', '.'));
      if (Number.isFinite(prixAvant)) {
        if (!Number.isFinite(prix) || prixAvant > prix) {
          payload.prixAvant = prixAvant;
        }
      }
    }

    var variants = record.variants !== undefined ? parseVariantsString(record.variants) : null;
    if (variants) payload.variants = variants;

    var stockValue = null;
    if (record.stock !== undefined) {
      stockValue = normalizeStockValue(record.stock);
    }
    if (variants && stockValue === null) {
      stockValue = variants.reduce(function(sum, variant) {
        return sum + normalizeStockValue(variant.stock);
      }, 0);
    }
    if (stockValue !== null) {
      ...
      payload.stock = variants ? variants.reduce(function(sum, variant) {
        return sum + normalizeStockValue(variant.stock);
      }, 0) : 0;
    }

    if (record.lowStockThreshold !== undefined) {
      var lowStockValue = parseInt(record.lowStockThreshold, 10);
      if (Number.isFinite(lowStockValue) && lowStockValue >= 0) {
        payload.lowStockThreshold = lowStockValue;
      }
    }

    var images = record.images !== undefined ? parseImagesString(record.images) : [];
    var image = record.image ? String(record.image).trim() : '';
    if (images.length) payload.images = images;
    if (image) payload.image = image;
    if (!image && images.length) payload.image = images[0];
    payload.vendeurId = parseInt(vendeurId);

    payloads.push({ payload: payload, productId: productId, rowNumber: rowIndex + 2 });
  });

  if (!payloads.length) {
    setImportStatus('Aucune ligne valide a importer.', 'error');
    return;
  }
  ...
  for (var i = 0; i < payloads.length; i++) {
    ...
    var url = entry.productId ? '/api/products/' + entry.productId : '/api/products';
    var method = entry.productId ? 'PUT' : 'POST';
    setImportStatus('Import en cours (' + (i + 1) + '/' + payloads.length + ')...', '');
    try {
      var payloadToSend = entry.payload;
      if (entry.productId) {
        var currentRes = await apiFetch('/api/products/' + entry.productId);
        if (!currentRes.ok) {
          errors.push('Ligne ' + entry.rowNumber + ': produit introuvable.');
          continue;
        }
        var current = await currentRes.json();
        payloadToSend = {
          nom: current.nom,
          categorie: current.categorie,
          description: current.description,
          prix: current.prix,
          prixAvant: current.prixAvant,
          image: current.image,
          images: current.images,
          stock: current.stock,
          lowStockThreshold: current.lowStockThreshold,
          variants: current.variants,
          vendeurId: current.vendeurId
        };
        Object.assign(payloadToSend, entry.payload);
      }
      ...
      if (res.ok) {
        ...
        errors.push('Ligne ' + entry.rowNumber + ': ' + (errPayload.error || 'erreur import'));
      }
    } catch (error) {
      errors.push('Ligne ' + entry.rowNumber + ': erreur reseau');
    }
  }

  if (success) {
    var statusType = errors.length ? 'error' : 'success';
    setImportStatus('Import termine: ' + success + ' produit(s), ' + errors.length + ' erreur(s).', statusType);
    loadData();
  } else {
    setImportStatus('Import echoue. ' + errors.length + ' erreur(s).', 'error');
  }
